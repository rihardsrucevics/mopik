...
import { ChatMessageSchema, RidePlanSchema, nextPlanPrompt, planSummary, type RidePlan } from "@/lib/chat/ride-plan";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { language, clarification } = result.parsed_output;
    ...
    const next = nextPlanPrompt(plan, language === "lv");
    const clarificationText = next ? null : clarification?.trim();
    const acknowledgement = describeChanges(previous, plan, language === "lv");
    const question = next?.message || clarificationText;
    const message = [acknowledgement, question || `${language === "lv" ? "Plānoju braucienu" : "Planning your ride"}: ${planSummary(plan, language === "lv")}.`].filter(Boolean).join("\n\n");
