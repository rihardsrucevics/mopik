...
import { seawardVias } from "../lib/routing/seaward";
 ...
 */
function viaCandidates(ride: Ride): { variant: string; points: [number, number][] }[] {
  const a = ride.from;
  const b = ride.to!;
  ...
  const reach = Math.min(20000, Math.max(3000, spareMeters / (2 * 5), legMeters * 0.12));

  const out: { variant: string; points: [number, number][] }[] = [];
  for (const scale of [0, 0.35, 0.7, 1, 1.4, 1.8, 2.2, 2.8]) {
    for (const side of scale === 0 ? [1] : [1, -1]) {
      const points: [number, number][] = [[a.lon, a.lat]];
      if (scale) points.push(perpendicularVia(a, b, 0.5, reach * scale * side));
      points.push([b.lon, b.lat]);
      out.push({ variant: `via-${scale}-${side}`, points });
    }
  }
  for (const [scale] of [[1.2], [0.7]] as const) {
    ...
    points.push([b.lon, b.lat]);
    out.push({ variant: `zig-${scale}`, points });
  }
  return out;
}
